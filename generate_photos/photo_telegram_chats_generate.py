...
logger_tg_chats_generate = logging.getLogger(__name__)


async def tg_chats_generate(profile_dict):
    """Функция для генерации картинки чатов телеграм с использованием шаблона"""
    ...
    try:
        ...
        while num_photo_chat != 4:
            path_inst_chat = f'../searchforphotos/images/tg_chats_{num_photo_chat}.png'
            # path_inst_chat = f'../images/tg_chats_1.png'
            ...
            if len(text) > 26:
                text = text[:26]
            # text_position = (147, background.size[1] - 1187)
            ...
            area_width = 150
            new_text_position_x = 215 + (area_width - text_width) / 2
            text_position = (new_text_position_x, background.size[1] - 1190)
            text_color = (255, 253, 253)
            draw.text(text_position, text, fill=text_color, font=font)
            ...
            #background.save('res_tg_chats.png', format='PNG')
            ...
            logger_tg_chats_generate.info('Картинка сгенерирована')

            tg_photo_list_objects.append(image_png_file_buffer)

        return tg_photo_list_objects
    except Exception:
        logger_tg_chats_generate.info('Не удалось сгенерировать фотографии чата тг')
        return None
...
#     asyncio.run(tg_chats_generate(say_dict))
